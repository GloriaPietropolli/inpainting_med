...
train_dataset = Normalization_Float(data_sat, mean_tensor, std_tensor)
...
losses = []  # losses of the completion network during phase 1
losses_test = []  # losses of TEST of the completion network during phase 1
...
for ep in range(epoch1):
    ...
    if ep % snaperiod == 0:
        ...
        with torch.no_grad():
            ...
            display.clear_output(wait=True)
            f_test.write(f"[EPOCH]: {ep + 1}, [LOSS]: {loss_test.item():.12f} \n")
...
f.close()
f_test.close()
Plot_Error(losses_test, '1c', path_lr + '/')  # plot of the error in phase1
