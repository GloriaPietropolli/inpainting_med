...
for variable in list(dict_channel.keys()):
    ...
    for month in months:  # iteration among months
        if month[-1] == "0":
            ...
            # month = "12"
        datetime = "2015." + month
        data_tensor = os.getcwd() + '/tensor/model2015_n/datetime_' + str(datetime) + '.pt'  # get the data_tensor correspondent to the datetime of emodnet sample to feed the nn (
            # NORMALIZED!!)
        ...
        # plt.plot(means_phys, range(len(means_phys)), color="slategray", linestyle='--', marker='v', alpha=0.8,
        ...
        plt.savefig(path_fig + '/' + variable + '_pro_mean_' + month + '.png')
        plt.close()
        ...
        plt.savefig(path_fig + '/' + variable + '_pro_std_' + month + '.png')
        plt.close()
